...
| A specific task or issue needs ongoing execution ownership | [supervise](https://aihero.dev/skills-supervise), which assigns each delegated ticket its own desktop thread, asks for worker model and effort with Sol / Medium as the default, verifies results, and keeps the parent objective open until acceptance is established |
