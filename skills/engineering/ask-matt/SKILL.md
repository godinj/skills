...
- **`/supervise`** owns a specific task or issue through verified completion: keep the current desktop thread as supervisor, give each delegated ticket its own dedicated desktop thread, inspect results, and choose the next useful action against the parent objective. Subagents do not replace those worker threads. It asks once for the worker model and effort, offering Sol / Medium by default. Use it for ongoing execution ownership; a one-time status check stays a status check. It calls `/reassess` when repairs stop advancing the goal. Existing planning limits, permissions, and human decisions still govern the assignment.
- **`/reassess`**: reassess the approach and mental model behind work already underway. Reach for it when repeated local fixes stop helping, evidence challenges a working assumption, a previously working approach regresses after redesign, or repair tasks succeed while the goal stays blocked. Model-invoked: agents can use it autonomously, and `/implement` and `/wayfinder` call it when progress or evidence challenges the approach. It names an assessment level, from local tactic through approach, interactions, problem framing, or goal, and widens only as the evidence requires. It compares an earlier working approach when available and examines whether repair evidence supports another attempt. Before an expensive attempt, it checks cheaply inspectable prerequisites through the next result's accepting consumer. It can separate tangled uncertainties into testable subgoals while retaining an integration check; invoke `/to-tickets` if you want that result turned into tracked execution work. It returns an evidence-based verdict and one concrete next step to the current flow, preserving its scope and completion rules.
